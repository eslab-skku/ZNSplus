...
#include "ssd/ioub_util.h"
#include "ssd/bast_util.h"

typedef struct NvmeTempCmd {
	int num;
	QTAILQ_ENTRY(NvmeTempCmd) entry;
} NvmeTempCmd;
...
typedef struct NvmeRequest {
    ...
    int64_t                 st;
    int64_t					cmd_idx;
	uint64_t		cmd_fetch_time;
	uint64_t		dispatch_comp_time;
	uint64_t		flash_comp_time;
	int completed;
} NvmeRequest;
...
typedef struct NvmeSQueue {
    ...
    QTAILQ_HEAD(sq_cmd_list, NvmeTempCmd) cmd_list;
    ...
    uint32_t	last_sq_index;	
    uint32_t	*head_bitmap;	
} NvmeSQueue;
