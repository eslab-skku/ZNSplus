		...
		unsigned int block_nb, unsigned int page_nb, nand_io_info* n_io_info)
{
	...
	uint64_t *chnl_next_avail_time = ssd->chnl_next_avail_time;
	uint64_t *chip_next_avail_time = ssd->chip_next_avail_time;
	uint64_t now = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
	uint64_t cur_need_to_emulate_tt = 0;
	...
	if (now < chip_next_avail_time[num_flash]) {
		start_time = chip_next_avail_time[num_flash];
		chip_next_avail_time[num_flash] += sc->CELL_PROGRAM_DELAY + sc->REG_WRITE_DELAY;
	} else {
		start_time = now;
		chip_next_avail_time[num_flash] = now + sc->CELL_PROGRAM_DELAY + sc->REG_WRITE_DELAY;
	}
	cur_need_to_emulate_tt = chip_next_avail_time[num_flash];
		...
		unsigned int block_nb, unsigned int page_nb, nand_io_info* n_io_info)
{
	...
	if (now < chip_next_avail_time[num_flash]) {
		start_time = chip_next_avail_time[num_flash];
		chip_next_avail_time[num_flash] += sc->CELL_READ_DELAY + sc->REG_READ_DELAY;

	} else {
		start_time = now;
		chip_next_avail_time[num_flash] = now + sc->CELL_READ_DELAY + sc->REG_READ_DELAY;
	}
	cur_need_to_emulate_tt = chip_next_avail_time[num_flash];
...
int64_t SSD_PAGE_COPYBACK(struct ssdstate *ssd, unsigned int flash_nb, unsigned int src_block_nb, unsigned int src_page_nb, unsigned int dest_block_nb, unsigned int dest_page_nb)
{
	struct ssdconf *sc = &(ssd->ssdparams);
	int CHANNEL_NB = sc->CHANNEL_NB;
	int PLANES_PER_FLASH = sc->PLANES_PER_FLASH;
	...
	if (now < chip_next_avail_time[num_flash]) {
		start_time = chip_next_avail_time[num_flash];
		chip_next_avail_time[num_flash] += sc->CELL_READ_DELAY + sc->CELL_PROGRAM_DELAY;
	}
	else {
		start_time = now;
		chip_next_avail_time[num_flash] = now + sc->CELL_READ_DELAY + sc->CELL_PROGRAM_DELAY;
	}
	cur_need_to_emulate_tt = chip_next_avail_time[num_flash];
