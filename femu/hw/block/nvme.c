...
uint16_t nvme_dma_read_prp(NvmeCtrl *n, uint8_t *ptr, uint32_t len,
        uint64_t prp1, uint64_t prp2);
void nvme_update_sq_head(NvmeSQueue *sq);
uint16_t nvme_io_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req);
void nvme_enqueue_req_completion_io(NvmeCQueue *cq, NvmeRequest *req);

struct IOUBprofile *prof_IOUB = NULL;

uint64_t DMA_next_avail_time = 0;

static void nvme_process_io(NvmeSQueue *sq, NvmeCQueue *cq, NvmeRequest *req, NvmeCmd *cmd)
{
    NvmeCtrl *n = sq->ctrl;
    uint16_t status;
	...
	if (status != NVME_NO_COMPLETE) {
		...
		sq->head_bitmap[req->cmd_idx] = 3;
	} 

	if (req->cmd_idx == sq->head) nvme_update_sq_head(sq);
}

static uint32_t GetFirstInvalid(struct ssdstate *ssd, struct act_blk_info *pact)
{
...
static struct req_info *get_req_info(struct ssdstate *ssd, NvmeCtrl *n, NvmeRequest *req)
{
	dma_addr_t sg_cur_byte = 0;
	int64_t sg_cur_index = 0;
	dma_addr_t cur_addr, cur_len;
	DMADirection dir = req->is_write ? DMA_DIRECTION_TO_DEVICE : DMA_DIRECTION_FROM_DEVICE;

	ssd->req_info->length = 0;
	ssd->req_info->p_hs = n->heap_storage;
	ssd->req_info->hs_oft = req->data_offset;
	
	void *mem;
	ssd->req_info->p_data = g_malloc0(req->qsg.nsg * 4096);
	...
	while (sg_cur_index < req->qsg.nsg) {
		...
		memcpy(ssd->req_info->p_data + sg_cur_byte, mem, cur_len);
        ...
        NvmeCmd *cmd, NvmeRequest *req)
{
	...
	if((req->data_offset >> 9) == ADDR_INTERNAL_COMPACTION) {
		// Copy data for FEMU
		uint32_t offset;
		uint32_t *data_addr = ssd->req_info->p_data;

		while(1) {
			uint64_t src_lpn, dst_lpn;			

			if(offset == IOUB_MAX_IM_CNT) break;

			src_lpn = data_addr[offset]; 
			dst_lpn = data_addr[512 + offset];
			...
			memcpy(hs + dst_lpn * sc->LPAGE_SIZE, hs + src_lpn * sc->LPAGE_SIZE, sc->LPAGE_SIZE);
			offset++;
...
void nvme_update_sq_head(NvmeSQueue *sq)
{
	int idx = sq->head % sq->size;
	while (idx != sq->tail) {
		if (sq->head_bitmap[idx] != 3)
			break;
		sq->head_bitmap[idx] = 0;
		idx = (idx + 1) % sq->size;
	}
	sq->head = idx;
}
    ...
    NvmeRequest *req)
{
    ...
    if (req->is_write) {
	uint64_t flash_time = 0, old_expire_time = req->expire_time;
	struct ssdconf *sc = &(ssd->ssdparams);
	get_req_info(ssd, n, req);
	...
	uint64_t start_DMA_ts;
	uint64_t flash_comp_ts;
	struct ssdconf *sc = &(ssd->ssdparams);

	if (n->femu_mode == FEMU_BLACKBOX_MODE) {
		dma_time = data_size / NVME_MAX_BANDWIDTH;
		if(now < DMA_next_avail_time) {
			start_DMA_ts = DMA_next_avail_time;
		}
		else {
			start_DMA_ts = now;
		}
		req->expire_time = DMA_next_avail_time = start_DMA_ts + dma_time;
	}
    ...
    int contig)
{
    ...
    sq->io_cmd = g_malloc0(sq->size * sizeof(*sq->io_cmd));
    sq->head_bitmap = g_malloc0(sq->size * sizeof(*sq->head_bitmap));
    QTAILQ_INIT(&sq->req_list);
    QTAILQ_INIT(&sq->out_req_list);
    QTAILQ_INIT(&sq->cmd_list);
    for (i = 0; i < sq->size; i++) {
        sq->io_req[i].sq = sq;
	sq->io_req[i].cmd_idx = i;
        QTAILQ_INSERT_TAIL(&(sq->req_list), &sq->io_req[i], entry);
		sq->io_cmd[i].num = i;
		sq->head_bitmap[i] = 0;
	sq->io_req[i].completed = 1;
    }
...
static void nvme_process_sq_io(void *opaque)
{
	...
	uint64_t bank_lpn = 0;
	uint64_t lbn = 0;
	uint64_t assoc_type = 0;
	uint64_t off_in_page = 0;
	struct act_blk_info *pact = NULL;
	uint32_t *sq_buf = NULL;
	uint32_t cmd_off_iter;
	static uint32_t SC_start = 0;

	static uint64_t next_util_check = 0;
	static uint64_t next_print = 0, now;

	if(next_util_check == 0)
		next_util_check = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);

	if(next_print == 0)
                next_print = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);

	nvme_update_sq_tail(sq);

	if(prof_IOUB == NULL) {
		prof_IOUB = (IOUBprofile*)malloc(sizeof(struct IOUBprofile));
	...
	if (!(nvme_sq_empty(sq) || QTAILQ_EMPTY(&sq->req_list)) && processed++ < sq->arb_burst) {
		for(cmd_off_iter = sq->head; cmd_off_iter != sq->tail; cmd_off_iter = (cmd_off_iter + 1) % sq->size) {
			uint32_t req_idx;

			req = &sq->io_req[cmd_off_iter];
			req_idx = req->cmd_idx;

			cmd = &req->cmd;	

			if (sq->phys_contig)
				addr = sq->dma_addr + req_idx * n->sqe_size;
			else
				addr = nvme_discontig(sq->prp_list, req_idx, n->page_size, n->sqe_size);
			nvme_addr_read(n, addr, (void *)&req->cmd, sizeof(struct NvmeCmd));

			if (req->cmd.opcode == NVME_OP_ABORTED) {
				continue;
			}

			if(sq->head_bitmap[req->cmd_idx] == 0 && req->completed == 1) {
				cmd = &req->cmd;

				sq->head_bitmap[req->cmd_idx] = 1;
				req->cmd_fetch_time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
				req->dispatch_comp_time = 0;
				req->flash_comp_time = 0;
				req->completed = 0;
			} else if(sq->head_bitmap[req->cmd_idx] == 0 && req->completed == 0) {
				break;
			}
		}
		...
		for(cmd_off_iter = sq->head; cmd_off_iter != sq->tail; cmd_off_iter = (cmd_off_iter + 1) % sq->size) {
			...
			if (sq->head_bitmap[req->cmd_idx] == 0 || sq->head_bitmap[req->cmd_idx] == 3) {
				continue;
			}
			...
			if (cmd->opcode == NVME_CMD_WRITE) {
				...
				if(sq->head_bitmap[req->cmd_idx] == 1) {
					uint64_t now = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
					if(req->dispatch_comp_time == 0) {
						uint64_t start_data_transfer_ts = 0;
						uint64_t transfer_delay = data_size / NVME_MAX_BANDWIDTH;
						//The write command performs host-to-device DMA first.

						if(now < DMA_next_avail_time) {
							start_data_transfer_ts = DMA_next_avail_time;
						} else {
							start_data_transfer_ts = now;
						}
						DMA_next_avail_time = req->dispatch_comp_time = start_data_transfer_ts + transfer_delay;
					}

					if(now >= req->dispatch_comp_time) {
						sq->head_bitmap[req->cmd_idx] = 2;
					} else {
						continue;
					}
				}

				sector_nb = data_offset >> 9;

				lpn = sector_nb / sc->SECTORS_PER_PAGE;
				bank = lpn % sc->NUM_BANKS;
				bank_lpn = lpn / sc->NUM_BANKS;
				lbn = bank_lpn / sc->PAGE_NB;
				assoc_type = get_assoc_type(ssd, bank, lbn);
				off_in_page = (sector_nb / sc->SECTORS_PER_LPAGE) % sc->LPAGE_PER_PPAGE;

				//This simulator makes special commands using write commands.
				if (sector_nb == ADDR_INTERNAL_COMPACTION) {
					//Internal Zone Compaction
					int32_t dst_lpn, dst_page;

					nvme_map_prp(&req->qsg, &req->iov, prp1, prp2, data_size, n);
					get_req_info(ssd, n, req);

					dst_lpn = ssd->req_info->p_data[512] + PARTITION_SIZE_IN_SECTOR / sc->SECTORS_PER_LPAGE;
					dst_page = dst_lpn / sc->LPAGE_PER_PPAGE;

					bank = get_num_bank(ssd, dst_page);
					bank_lpn = get_num_stripe(ssd, dst_page);
					lbn = bank_lpn / sc->PAGE_NB;
					...
					if (pact) {
						if (GetFirstInvalid(ssd, pact) == dst_lpn % (sc->PAGE_NB * sc->LPAGE_PER_PPAGE * sc->NUM_BANKS)) {
							//This command has been scheduled
							nvme_process_io(sq, cq, req, cmd);
							processed++;
							prof_IOUB->IZC_count++;
							prof_IOUB->IZC_total_time += req->expire_time - req->cmd_fetch_time;
							break;
						}
					}
				}
				else if (sector_nb == ADDR_TL_OPEN && ssd->ioub_start >= sc->NUM_ACT_BLKS) {
					//Section CLose // TL_Open
					SC_start = 1;
					nvme_map_prp(&req->qsg, &req->iov, prp1, prp2, data_size, n);							
					get_req_info(ssd, n, req);

					lbn = ssd->req_info->p_data[2] + ssd->n_page_mapping_lbn;

					g_free(ssd->req_info->p_data);
					if (pact = actblk_find(ssd, 0, lbn)) {
						if (GetFirstInvalid(ssd, pact) == sc->PAGE_NB * sc->LPAGE_PER_PPAGE * sc->NUM_BANKS) {
							//This command has been scheduled
							nvme_process_io(sq, cq, req, cmd);
							processed++;
							break;
				...
				else if (assoc_type == ASSOC_BLOCK && ssd->ioub_start > 0) {
					//Normal Write
					if (pact = actblk_find(ssd, bank, lbn)) {

						if (GetFirstInvalid(ssd, pact) == (lpn * sc->LPAGE_PER_PPAGE + off_in_page) % (sc->LPAGE_PER_PPAGE * sc->PAGE_NB * sc->NUM_BANKS)) {
							////This command has been scheduled
							nvme_process_io(sq, cq, req, cmd);
							processed++;
							break;
				...
				if(cmd->opcode == NVME_CMD_READ && SC_start == 1 && req->flash_comp_time == 0) {
					if (pact = actblk_find(ssd, bank, lbn)) {
						uint64_t page_offset;
						page_offset = (sector_nb / sc->SECTORS_PER_LPAGE) % (sc->LPAGE_PER_PPAGE * sc->PAGE_NB * sc->NUM_BANKS);
						page_offset /= sc->LPAGE_PER_PPAGE;

						if(!(pact->page_validity[page_offset] & (1 << off_in_page))) {
							//This command should not be scheduled yet.
							continue;
						}
					}
				}
				if(cmd->opcode == NVME_CMD_READ) {
					uint64_t now = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
					if(req->flash_comp_time == 0) {
						req->flash_comp_time = SSD_READ(ssd, data_size >> 9, data_offset >> 9);
						sq->head_bitmap[req->cmd_idx] = 2;
					}
					if(req->flash_comp_time > now) 
						continue;
				}
other:
				nvme_process_io(sq, cq, req, cmd); 
...
#ifndef PRE_PD_OFF
	if(SC_start)
		padding_all_actblk(ssd, qemu_clock_get_ns(QEMU_CLOCK_REALTIME));
#endif
#ifdef CHIP_UTIL_CHECK
	check_chip_utilization(ssd, qemu_clock_get_ns(QEMU_CLOCK_REALTIME));
#endif
#ifdef DEBUG_CHIP_UTIL
	now = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
	if(next_print < now) {
		if(prof_IOUB->CH_total_num == 0 )
			prof_IOUB->CH_total_num = 1;
		printf("CH	%f	%llu\n", (float)prof_IOUB->CH_total_active / prof_IOUB->CH_total_num, now);

		next_print += 100000000;
