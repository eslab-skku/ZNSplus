			 ...
			 const struct blk_mq_queue_data *bd)
{
	...
	spin_unlock_irq(&nvmeq->q_lock);
//	if(req->bio)
//	printk("	ENQUEUE - addr %u-%u - %u	sq-%u\n", req->bio->bi_iter.bi_sector / 8 / 2048, (req->bio->bi_iter.bi_sector / 8) % 2048, req->bio->bi_iter.bi_size / 4096, nvmeq->sq_tail);
	return BLK_MQ_RQ_QUEUE_OK;
out_cleanup_iod:
	nvme_free_iod(dev, req);
out_free_cmd:
...
//	if(req->bio)
//		printk("	DEQUEUE - addr %u-%u\n", req->bio->bi_iter.bi_sector / 8 / 2048, (req->bio->bi_iter.bi_sector / 8) % 2048);
